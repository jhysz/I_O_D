...
### "numbers.txt" 中的时间 和 "eph.npy" 的星历 都是由matlab仿真得到的
obs1 = Data('obs_simu_1.txt')
# [R, L, T]
T = obs1[2]
print(T)
print(obs1[0])

## 观测点在地固系下的 笛卡尔坐标 ： （lat：-60 lon：30  alt：30）
site = [2768786.78121295, 1598559.7934619738, -5500503.114700751]

## 得到测站随时间变化的GCRS坐标
obs_pos_tmp = []
for tt in T:
    site_t = Coordinate(tt)
    obs_pos = site_t.ITRS_to_ICRS(site[0], site[1], site[2])
    obs_pos_tmp.append(list(obs_pos))

print(np.array(obs_pos_tmp).shape)
print(obs_pos_tmp)
...
### 只取前30个
eph = eph[:30]
eph_ = eph[:,:3] - np.array(obs_pos_tmp)
print(eph_.shape)
...
for i in range(num_prop):
    now = Time(T[i])
    # position of satellite in GCRS:
    cartrep = coord.CartesianRepresentation(x=eph_[i,0],
                                            y=eph_[i,1],
                                            z=eph_[i,2], unit=u.m)
    gcrs = coord.GCRS(cartrep) #,obstime=now)

    ra = gcrs.ra.to(u.deg).to_value()
    dec = gcrs.dec.to(u.deg).to_value()
    ra_tmp.append(ra)
    dec_tmp.append(dec)

## 将 ra dec 存入 numbers 即得到输入数据

print(ra_tmp)
print(dec_tmp)
np.save(file = 'ra_tmp.npy',arr=ra_tmp)
np.save(file = 'dec_tmp.npy',arr=dec_tmp)